let isAdmin = false;
let icons = [
    { icon: "💧", text: "Água", speak: "Eu quero água", avatar: "🥤", isImage: false },
...
// Biblioteca estilo WhatsApp (Mais completa)
const BIBLIOTECA = [
    "😀","😂","😊","😍","🤔","😴","🤯","🤒","😎","🥳","😭",
    "👍","👋","🙌","👏","🙏","🤝","❤️","💧","🍽️","🥣","🥤",
    "🍎","🍌","🍕","🥪","🍪","⚽","🎨","📚","🧩","🏠","🏫",
...
function clicarAcao(item) {
    ...
    avatar.innerHTML = item.isImage ? `<img src="${item.icon}" style="width:100px; height:100px; border-radius:50%;">` : item.icon;
...
function abrirModalPersonalizar() {
    ...
    BIBLIOTECA.forEach(e => {
        ...
        d.innerText = e; d.style.cursor="pointer"; d.style.fontSize="1.5rem";
        d.onclick = () => {
...
function salvarNovoCard() {
    ...
    const isImg = document.getElementById('is-image-type').value === "true";
    if(icon && text && speak) {
        icons.push({ icon, text, speak, isImage: isImg });
        renderizarCards();
        fecharModal();
    }
}
